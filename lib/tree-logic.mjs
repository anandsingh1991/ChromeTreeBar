...
/**
 * Display label for a group with no user-set title. Chrome shows the shared hostname
 * when every tab is on one site; there is no single host for a mixed group, so fall
 * back to a count. Never returns '' so a group row is always identifiable.
 *
 * Takes every tab in the group, nested ones included — group membership is flat in
 * Chrome, and a group can be formed entirely from nested tabs.
 * @param {Array<{url?:string}>} tabs
 */
export function untitledGroupLabel(tabs) {
  const list = tabs || [];
  if (list.length === 0) return 'Empty group';
  ...
  for (const t of list) {
    ...
    try {
      ...
      // Only http(s) has a meaningful site host: chrome://newtab/ parses with
      // hostname 'newtab', which would read as a real shared site.
  ...
  const [only] = hosts;
  if (hosts.size === 1 && only) return only;
  return list.length === 1 ? '1 tab' : `${list.length} tabs`;
}
